...
public class 收品项目 {
    ...
    // 分库分表如何快速扫描全量数据？
    // 方案1：
    //  1、报名人表 按活动纬度分库，64个库就有64个表，活动code分片，先把所有人扫出来写入到任务表（初始状态待处理）
    //  2、定时任务5秒扫描500条数据出来执行，每台机器限制16个线程执行（redis可动态控制），单个人处理前加锁，
    //        拿到活动code+报名人，就定位到sku的表，扫出所有这个报名人下的已报sku数据发出mq
    //  3、mq批量拉取消费，同步到下游系统
    // 方案2：64个库，256个表，64x256总共生成这么多个任务，1个表开启一个任务去扫描sku然后发出mq
